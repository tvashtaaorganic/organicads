...
const formatSize = (bytes) => {
  ...
  export default function ImageOptimizer() {
        ...
        try {
          ...
          if (data.error) {
            ...
            const mergedStats = {
              ...
              imagesConverted: Math.max(data.stats.imagesConverted, JSON.parse(savedStats || '{"imagesConverted": 0}').imagesConverted),
              totalSizeReduced: Math.max(data.stats.totalSizeReduced, JSON.parse(savedStats || '{"totalSizeReduced": 0}').totalSizeReduced),
            };
    ...
    const optimizeImage = async () => {
      ...
      formData.append("reductionPercent", reductionPercent);
    ...
    useEffect(() => {
      ...
      if (sliderRange) {
        ...
        sliderRange.addEventListener('input', handleSliderChange);
        sliderRange.addEventListener('change', handleSliderChange);
      ...
      return () => {
        if (sliderRange) {
          ...
          sliderRange.removeEventListener('input', handleSliderChange);
          sliderRange.removeEventListener('change', handleSliderChange);
  ...
  {result && (
    ...
    >
      <AlertTitle>
        {result.type === "error" ? "Error" : "Optimization Result"}
      </AlertTitle>
      <AlertDescription>
        {result.type === "error" ? (
          result.message
          ...
          <>
            ...
            <p>
              <strong>Saved:</strong> {result.savings}% ({((result.originalSize - result.compressedSize) / 1024).toFixed(2)} KB)
            </p>
            <a
...
<section className="image-comparison max-w-[48.063em] mx-auto mt-6" data-component="image-comparison-slider">
  <div className="image-comparison__slider-wrapper relative">
    <label
      htmlFor="image-comparison-range"
      className="image-comparison__label text-gray-700 text-lg text-center font-semibold mb-2 block z-20" // Removed sr-only to make it visible, adjust styling as needed
    >
    ...
    <div className="image-comparison__image-wrapper relative pt-[66.666666667%]">
      <figure className="image-comparison__figure absolute top-0 left-0 w-full h-full">
        ...
        <figcaption className="image-comparison__caption image-comparison__caption--before absolute bottom-3 left-3 text-left min-w-max flex flex-col text-white font-bold uppercase text-sm md:text-base z-20">
          <span className="image-comparison__caption-body px-3 py-1.5 bg-black/55">Before Label</span>
        </figcaption>
        <figcaption className="image-comparison__caption image-comparison__caption--after absolute bottom-3 right-3 text-right min-w-max flex flex-col text-white font-bold uppercase text-sm md:text-base z-20">
          <span className="image-comparison__caption-body px-3 py-1.5 bg-black/55">After Label</span>
        </figcaption>
      </figure>
    </div>
  </div>
